//! Filesystem locations for ClosedHands config files and binaries.

use std::path::PathBuf;
use std::sync::OnceLock;

static CLOSEDHANDS_HOME: OnceLock<PathBuf> = OnceLock::new();
...
/// The default user grok directory (`~/.closedhands`, canonicalized) used when
/// `CLOSEDHANDS_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
...
/// `~/.closedhands/marketplace-cache`. `dunce` strips the prefix whenever the path
...
pub fn default_grok_home() -> PathBuf {
    ...
    dunce::canonicalize(&home).unwrap_or(home).join(".closedhands")
}

/// Per-user config directory: `$CLOSEDHANDS_HOME` or `~/.closedhands`. Created if needed.
pub fn grok_home() -> PathBuf {
    CLOSEDHANDS_HOME
        .get_or_init(|| {
            let grok_home = if let Ok(v) = std::env::var("CLOSEDHANDS_HOME") {
                PathBuf::from(v)
            } else {
                default_grok_home()
            };
...
/// `$CLOSEDHANDS_HOME` is set or a home directory is found, `None` otherwise. Unlike
...
pub fn user_grok_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("CLOSEDHANDS_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(grok_home)
}

/// Canonical grok application path: `$CLOSEDHANDS_HOME/bin/grok` (Unix) or `grok.exe` (Windows).
pub fn grok_application() -> PathBuf {
    grok_application_in(&grok_home())
}

/// [`grok_application`] under an explicit home instead of `$CLOSEDHANDS_HOME`.
pub fn grok_application_in(home: &std::path::Path) -> PathBuf {
    let name = if cfg!(windows) { "closedhands.exe" } else { "closedhands" };
...
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/closedhands"))
    } else {
        None
    }
}
...
mod tests {
    ...
    #[test]
    fn default_closedhands_home_has_no_verbatim_prefix() {
        ...
        assert!(home.ends_with(".closedhands"));
    }

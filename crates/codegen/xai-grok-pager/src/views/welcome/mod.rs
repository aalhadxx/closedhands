...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **ClosedHands** VERSION+channel **Beta** (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | ClosedHands Beta [channel] (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **ClosedHands Beta**  VERSION (left-aligned).
    HeroInline,
}
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            let label = params.login_label.unwrap_or("ClosedHands");
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "ClosedHands is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// multi-line block showing the workspace path and the warning that ClosedHands
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "ClosedHands may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
    ...
    let (cursor_pos, post_flush_escapes) = if show_picker {
        ...
        if centered.height > 2 {
            ...
            let gate_link = p
                ...
                .unwrap_or("https://ollama.com?referrer=closedhands");

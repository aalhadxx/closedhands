//! `closedhands completions <shell>` — generate shell completion scripts.
//!
...
pub fn run(shell: Shell) {
    // Ensure the script always uses the public "closedhands" name (matches historical
    // behavior and what the installers + docs expect).
    let mut cmd = PagerArgs::command().name("closedhands");
    if shell != Shell::Zsh {
        generate(shell, &mut cmd, "closedhands", &mut std::io::stdout());
        return;
    }
    // zsh needs post-processing (see fix_zsh_root_prompt_positional).
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "closedhands", &mut buf);
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("closedhands");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "closedhands", &mut buf);

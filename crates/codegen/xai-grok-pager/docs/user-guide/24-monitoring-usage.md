...
- **Structurally separate** from the project-internal telemetry: its exporters carry
  only the headers you configure, never the project credentials.
- **Independent of the project data-retention opt-outs**: it works even when
  `telemetry` is disabled and for ZDR (zero-data-retention) teams. Those
  settings govern the project-side retention; the external stream is governed solely
...
|---|---|---|
| `GROK_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `GROK_TELEMETRY_ENABLED`, which controls the project-internal product analytics — the two govern opposite-pointing data flows. |

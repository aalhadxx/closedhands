...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.closedhands/agents/` or `~/.closedhands/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.closedhands/personas/`; applied during subagent resolution |
...
| **Examples** | `closedhands-build`, `explore`, `plan` | `researcher`, `concise` |
...
ClosedHands discovers file-based personas from these locations, in priority order:

- `.closedhands/personas/*.toml` (project)
- `~/.closedhands/personas/*.toml` (user)
...
> **Note:** ClosedHands applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, ClosedHands resolves the effective model and reasoning effort in this order, highest priority first:
...
ClosedHands manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "closedhands-build"               # route explore to a specific model
```
...
# instructions_file = ".closedhands/personas/concise.md"  # or load from a file
```

ClosedHands also discovers roles from `.closedhands/roles/*.toml` and personas from `.closedhands/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

ClosedHands shows running and finished work in side panes on the agent screen:
...
- `Subagent running: "do the thing" (Implementer · closedhands-3) — Thinking`

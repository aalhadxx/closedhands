...
Headless mode runs ClosedHands non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
ClosedHands processes the prompt, runs any necessary tools, and prints the result to stdout. The process exits when the response is complete.
...
| `-m, --model <MODEL>`   | Model to use (e.g., `closedhands-build`)              |
...
On failure, ClosedHands emits an error object (process exit non-zero). Prompt-level
...
ClosedHands may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.

---

## Session Management in Headless Mode

By default, each `closedhands -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
closedhands -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
closedhands -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
closedhands -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
closedhands -p "What's the secret number?" --resume abc123
```
...
closedhands -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
closedhands --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  closedhands -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
ClosedHands's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
class GrokChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["closedhands", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json",
                "--yolo"]

    async def create(self, messages, model="closedhands-build", stream=False):
...
RESULT=$(closedhands -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
```bash
closedhands -p "Format all files" --always-approve
closedhands -p "Run the tests and fix any failures" --cwd ~/projects/my-app --always-approve
```
...
closedhands -p "Run the test suite" --yolo
```
...
- **`closedhands login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`closedhands login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `closedhands -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug closedhands -p "..." 2> debug.log`.

---

## Project Root Discovery

When ClosedHands starts, it discovers the project root by walking upward from `--cwd`
...
ClosedHands discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
ClosedHands stores data in `~/.closedhands` (override with `GROK_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
### Read-Only `~/.closedhands`

For containers or CI, mount `~/.closedhands` read-only:
...
unless it runs from the managed install (`$GROK_HOME/bin/closedhands`).
...
- Resume: `closedhands -p "continue" --resume "<id>"` or `closedhands -p "continue" --continue`

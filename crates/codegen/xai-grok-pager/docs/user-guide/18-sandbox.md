...
```bash
# Run with workspace sandbox (read everywhere, write to CWD + temp dirs + ~/.closedhands/)
closedhands --sandbox workspace

# Read-only mode (read everywhere, write only to ~/.closedhands/ + temp dirs)
closedhands --sandbox read-only

# Most restrictive profile (read CWD + system paths, write CWD + temp dirs + ~/.closedhands/, no child network)
...
| `workspace`           | Everywhere         | CWD + `~/.closedhands/` + `/tmp` + `/var/tmp`         | Allowed       | Normal development                |
| `devbox`              | Everywhere         | All top-level dirs except `/data`              | Allowed       | Disposable dev VMs                |
| `read-only`           | Everywhere         | `~/.closedhands/` + `/tmp` + `/var/tmp`               | Blocked¹      | Exploration, code review          |
| `strict`              | CWD + system paths | CWD + `~/.closedhands/` + `/tmp` + `/var/tmp`         | Blocked¹      | Untrusted code                    |
...
**workspace** -- The recommended profile for everyday development. The agent can read any file on the system (for understanding dependencies, system libraries, etc.) but can only write to the current working directory, `~/.closedhands/`, and temp directories (`/tmp`, `/var/tmp`, plus the macOS temp dirs). Network access is allowed for tools like `web_search` and MCP servers.
...
**read-only** -- Use when you want the agent to analyze code without modifying your project files. The agent can read everything but can only write to `~/.closedhands/` (needed for session persistence) and temp directories. Child-process network access is blocked on Linux (no-op on macOS).

**strict** -- The most restrictive profile, for reviewing untrusted code. The agent can only read files within the current working directory and essential system paths. Writes are limited to CWD, `~/.closedhands/`, and temp directories. Child-process network access is blocked on Linux (no-op on macOS).

### Direct global hook write protection

Under `workspace`, `read-only`, and `strict` (and custom profiles that extend those bases), the ClosedHands state directory remains writable for session/runtime files, but the kernel **write-denies** the ClosedHands-owned direct disk paths used as user-global hook sources (they stay readable):

- `~/.closedhands/hooks/` (hook directory)
- `~/.closedhands/hooks-paths` (registry file; not loaded as hook JSON — only its absolute targets are)
- Absolute targets listed in `hooks-paths` (relative lines are ignored; missing targets refuse sandbox start)

On first launch under these profiles, ClosedHands creates a real empty `hooks/` directory and empty `hooks-paths` file when they are missing (never symlinks or wrong types). Claude/Cursor global settings are **not** covered by this write-deny; discovery of those vendors remains separately gated by compatibility settings.
...
Create custom sandbox profiles in `~/.closedhands/sandbox.toml` (global) or `.closedhands/sandbox.toml` (per-project):
...
If the user and project files define the same custom profile differently, ClosedHands uses the user profile and shows a startup warning. Run `/doctor` to see both file locations and how to resolve the conflict. Identical definitions do not produce a warning.
...
> `bubblewrap`: if it is missing (or any single deny path can't be bound), ClosedHands
...
> metacharacter, or one that is malformed, makes ClosedHands **refuse to start** (fail
...
>   so matching files are denied **even if created after ClosedHands starts**.
...
>   too many files, or whose tree is too deep/broad to walk, makes ClosedHands **refuse
...
The sandbox is applied to the **entire closedhands process** at startup using kernel primitives -- not per-command wrapping. This means all tool operations are covered:
...
- If a built-in profile fails to apply, ClosedHands warns and continues without enforcement (see [Platform Support](#platform-support)), but still refuses the leader so tools are not delegated elsewhere
- `closedhands workspace start`, `restart`, and `resume` are unavailable; `pause`, `stop`, and `status` still work
...
for the life of the session**. When you resume it (`closedhands --resume <id>`,
`closedhands --continue`, or `closedhands -r`), ClosedHands restores that same profile automatically —
...
If the sandbox cannot be applied (e.g., unsupported kernel, missing entitlements), ClosedHands logs a warning and continues without enforcement. The exception is an explicitly-requested **custom profile**: on **both macOS and Linux**, if it cannot be applied (unknown profile, malformed `sandbox.toml`, or — on Linux — `bubblewrap` unavailable for a non-empty `deny`), ClosedHands refuses to start rather than run with its denied paths exposed.
...
Sandbox events are logged to `~/.closedhands/sandbox-events.jsonl` for debugging. Events include:

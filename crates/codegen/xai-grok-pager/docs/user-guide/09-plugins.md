...
A plugin bundles skills, slash commands, agents, hooks, and MCP servers into one installable unit. You get plugins from a marketplace, install the ones you want, and ClosedHands loads what they add. To build and share your own, see [Create your own marketplace](#create-your-own-marketplace).
...
1. **Add the marketplace** so ClosedHands can show what it offers. Nothing installs yet.
...
```bash
closedhands plugin marketplace add my-org/team-plugins                  # GitHub shorthand (owner/repo)
closedhands plugin marketplace add https://gitlab.com/acme/plugins.git  # any git host, include https:// and .git
closedhands plugin marketplace add ./my-marketplace                     # a local folder
```

List, refresh, and remove sources with `closedhands plugin marketplace list`, `closedhands plugin marketplace update [<name>]`, and `closedhands plugin marketplace remove <url>`.
...
Place this file at `~/.closedhands/settings.json` or `~/.claude/settings.json`.
...
```bash
closedhands plugin install deploy-tools --trust
```
...
Run `closedhands plugin install <source>` without `--trust` and ClosedHands shows the source, warns that installing activates the plugin's hooks, MCP servers, and skills, then stops. Add `--trust` to go ahead. Only install plugins from sources you trust (see [Trust and security](#trust-and-security)).

A plugin's skills appear in the slash menu. When a skill name is ambiguous, ClosedHands shows the qualified form prefixed by the plugin name, for example `/deploy-tools:release`. To pick up a newly installed plugin, press `r` in the Plugins tab or start a new session.
...
```bash
closedhands plugin list [--json] [--available]   # installed plugins (--available requires --json)
closedhands plugin uninstall <name> [--confirm] [--keep-data]   # aliases: rm, remove
closedhands plugin update [<name>]               # omit the name to update every plugin
closedhands plugin enable <name>
closedhands plugin disable <name>
closedhands plugin details <name>                # show the plugin's component inventory
```
...
Set these in `~/.closedhands/config.toml`:
...
Plugins are off by default, so list one in `enabled` to turn it on, or in `disabled` to discover it but skip loading it. Each entry is a plain plugin name (from `closedhands plugin list`) or a full ID (`<scope>/<hash>/<name>`).

To hide the plugins and hooks interface entirely, set `disable_plugins = true` in `~/.closedhands/pager.toml`.
...
3. **List the plugins in `.closedhands-plugin/marketplace.json`.** This is the index ClosedHands reads.
...
my-org-plugins/
  .closedhands-plugin/
    marketplace.json      # the index ClosedHands reads (required)
...
ClosedHands reads the index from `.closedhands-plugin/marketplace.json`. It also accepts `.closedhands-plugin/plugin.json` and the `.claude-plugin/` equivalents.
...
Validate a plugin before publishing with `closedhands plugin validate [<path>]`, and tag a release from the manifest version with `closedhands plugin tag [<path>] [--push]`. Then point people at the repository. They add it once and install the plugins they want:

```bash
closedhands plugin marketplace add my-org/my-org-plugins   # GitHub shorthand, a git URL, or a local path
closedhands plugin install gdrive --trust
```
...
# Plugins stay off until enabled. List plugin names (from `closedhands plugin list`)
# or full IDs (`<scope>/<hash>/<name>`).
[plugins]
enabled = ["gdrive"]
```

For a hands-off install with no per-person step, also place the plugin's files where ClosedHands discovers and trusts them automatically: `~/.closedhands/plugins/`, or a directory your device-management tool manages that you point to with `[plugins].paths`. Then enable them with `[plugins].enabled`.
...
Marketplaces distribute ClosedHands content: skills, commands, agents, hooks, and MCP server configurations. They do not install a program onto a machine. A skill or MCP server that runs a helper binary (for example a custom sign-in tool) still needs that binary delivered separately, bundled with your deployment or pushed through your device-management tool.

---

## Troubleshooting

**A plugin you installed isn't showing up.** Plugins are off until enabled. Check `closedhands plugin list`, then add the plugin's name or ID to `[plugins].enabled`, or press `Space` on it in the Plugins tab. Reload with `r` in the Plugins tab or start a new session.

**A plugin's hooks or MCP servers don't run.** They stay inactive until the plugin is trusted. Reinstall with `--trust`, or place the plugin under `~/.closedhands/plugins/` (auto-trusted). See [Trust and security](#trust-and-security).

**A skill or MCP server from a marketplace is missing.** Refresh the source with `closedhands plugin marketplace update`, confirm the plugin is installed and enabled, and, if your organization restricts sources, check that the marketplace is still allowed (see [Distribute across an organization](#distribute-across-an-organization)). Some MCP servers require a sign-in and will not appear until you authenticate.
...
**See exactly what loaded.** Run `closedhands inspect` (add `--json` for machine-readable output) to list every discovered plugin and the skills, agents, hooks, and MCP servers it provides, each labeled with its `plugin: <name>` source.
...
An optional `plugin.json` manifest can override paths or add metadata; without one, ClosedHands discovers components from these standard directories. For example, a `team-tools` plugin might bundle a deploy skill, a code-review agent, pre-commit hooks, and a Linear MCP server, installed together in one step.
...
### Where ClosedHands looks for plugins

ClosedHands discovers plugins from these locations, in priority order. The `.claude/plugins/` equivalents also work, and when two plugins share a name the higher-priority one wins:

| Location | Scope | Trust |
|----------|-------|-------|
| `_meta.pluginDirs` (`session/new` / `session/load`) | Session, that session only | Trusted automatically |
| `--plugin-dir` (the `closedhands agent … stdio` flag) | Process, that agent process only | Trusted automatically |
| `.closedhands/plugins/` | Project, shared through version control | Requires trust |
| `~/.closedhands/plugins/` | User, every project | Trusted automatically |
| `[plugins].paths` (config) | Custom directories you add | Depends on location |

The `_meta.pluginDirs` field on the `session/new` and `session/load` requests loads plugins for a single session; because the caller supplies the directory, those plugins are trusted automatically and do not persist after the session. `--plugin-dir` is the process-wide equivalent for a dedicated `closedhands agent … stdio` process, repeatable (`closedhands agent --no-leader --plugin-dir A --plugin-dir B stdio`), and ignored in leader mode, where the shared leader discovers its own plugins.
...
ClosedHands sets these and overrides any same-named value in the hook's `env` map (the `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` aliases are set too). See the [Hooks guide](10-hooks.md) for every variable passed to hooks.

...
ClosedHands is a self-hosted terminal-based AI coding assistant. It runs as a TUI (Terminal User Interface) that understands your codebase, executes shell commands, edits files, searches the web, and manages tasks.
...
The PowerShell installer automatically adds `%USERPROFILE%\.closedhands\bin` to your User PATH. Alternatively, install via [Git for Windows](https://gitforwindows.org/) (Git Bash) or MSYS2 using the bash script above. WSL users get the Linux binary automatically.
...
Start ClosedHands by running:

```bash
closedhands
```

On first launch, ClosedHands opens your browser to authenticate with ollama.com. After you sign in, ClosedHands stores your credentials in `~/.closedhands/auth.json`, where they persist across sessions. ClosedHands refreshes your credentials automatically and prompts you to sign in again when they can no longer be renewed.
...
Once authenticated, ClosedHands presents a full-screen TUI with two main areas:

- **Scrollback** -- the conversation history showing your prompts, ClosedHands's responses, tool calls, file edits, and more.
- **Prompt** -- the input area at the bottom where you type messages.

Type a message and press `Enter` to send it. ClosedHands reads files, runs commands, and edits code as needed. Each tool run streams into the scrollback in real time.
...
By default, ClosedHands asks for permission before executing shell commands or editing files. You can approve individually or toggle always-approve mode:

- Press `Ctrl+O` to toggle always-approve mode
- Use the `--yolo` flag at launch: `closedhands --yolo`
...
Every conversation is a **session**. Sessions are automatically saved to `~/.closedhands/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.

- Start a new session: `Ctrl+N` or `/new`
- Resume a previous session: `/resume` in the TUI, or `--resume <ID>` from the CLI
- Continue the most recent session: `closedhands -c`
...
- **Agent messages** -- ClosedHands's responses with full markdown rendering and syntax highlighting
- **Thinking blocks** -- ClosedHands's reasoning process (collapsible)
...
ClosedHands has built-in tools for:
...
```
/model closedhands-build                 # Switch model
...
closedhands "fix the failing auth test and run it"

# Initial prompt in a new git worktree. Use --worktree=<name> (with `=`) so the
# prompt isn't swallowed as the worktree name — `closedhands -w "refactor module X"`
# would treat "refactor module X" as the worktree label, not the prompt.
closedhands --worktree=feat "refactor module X"

# Base the worktree on a specific branch (e.g. main) instead of the current HEAD:
closedhands -w --ref main "implement feature from main"
...
closedhands -m closedhands-build

# Resume a previous session
closedhands --resume <session-id>

# Continue the most recent session
closedhands -c

# Experimental scrollback-native render mode. Sticky: plain `closedhands` reopens in
...
Run ClosedHands non-interactively for scripting, CI/CD, and automation:
...
```bash
closedhands -p "Review changes for bugs" --output-format json --yolo | jq -r '.text'
```
...
Add per-project instructions by creating an `AGENTS.md` file in your repository. ClosedHands reads these files and injects their contents as a project-instructions message at the start of the conversation:

```
~/.closedhands/AGENTS.md           # Global rules (apply to all projects)
<repo-root>/AGENTS.md       # Repository-level rules
<cwd>/AGENTS.md             # Directory-level rules (highest priority)
```

Deeper files take precedence. ClosedHands also reads `CLAUDE.md` files for compatibility.

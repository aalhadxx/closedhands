...
- **`closedhands dashboard`** — launch the TUI into the dashboard.
...
```
 ClosedHands · Dashboard — 4 agents · 2 awaiting
...
State icons match other session lists in ClosedHands:
...
Per-user preferences under `[dashboard]` in `~/.closedhands/config.toml`:

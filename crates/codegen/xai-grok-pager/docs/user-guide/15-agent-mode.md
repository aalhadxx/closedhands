...
Agent mode runs ClosedHands as a long-lived server that clients talk to over [ACP](https://agentclientprotocol.com) (JSON-RPC). Use it from IDEs, SDKs, eval harnesses, and custom apps. For a one-shot prompt that prints and exits, use `closedhands -p` instead ([headless mode](14-headless-mode.md)).
...
The [Agent Client Protocol (ACP)](https://agentclientprotocol.com) defines how clients talk to coding agents over JSON-RPC. With ClosedHands it covers:
...
```bash
closedhands agent --always-approve --model closedhands-build stdio
closedhands agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```

| Flag | Description |
| ---- | ----------- |
| `-m, --model <MODEL>` | Model ID (for example `closedhands-build`). |
...
```bash
closedhands agent --always-approve headless --closedhands-ws-url wss://your-relay.example.com/ws
```
...
Beyond the base ACP protocol, ClosedHands defines extension methods under the `x.ai/` prefix for specific functionality. These cover:
...
The tables here show representative methods in each category. The `x.ai/*` set is specific to this implementation and may expand across releases, so treat it as non-exhaustive and discover the available methods from the agent's `initialize` response.
...
class GrokACPChat {
  ...
  async init() {
    this.proc = spawn("closedhands", ["agent", "--always-approve", "stdio"]);

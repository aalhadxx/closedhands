...
ClosedHands runs as a full-screen TUI. It relies on terminal support for color,
...
Run `/doctor` in ClosedHands to check the current session and see available fixes. If
ClosedHands cannot start, run `closedhands doctor` in your shell. Use `closedhands doctor --json`
...
`closedhands doctor --json` reports the same color capability when piped. Microphone
...
`/doctor fix tmux-clipboard` or `closedhands doctor fix dcs-passthrough --yes`.
...
to guess if that directory is unavailable or unsafe. ClosedHands preserves the file's
...
ClosedHands deliberately does **not** run `tmux source-file` or change the live tmux
...
ClosedHands detects these terminal emulators from environment variables:
...
- Inside tmux, variables that identify the outer terminal may not reach ClosedHands.
...
ClosedHands writes through up to three routes, shown in `/doctor` under **Clipboard**:

- **native** — the local operating-system clipboard.
- **tmux** — the tmux paste buffer when ClosedHands runs inside tmux.
...
terminal focused. Older compositors may require ClosedHands to remain focused until
the copy message appears. ClosedHands shows a startup warning when this applies; run
...
ClosedHands emits OSC 52 on Linux and across tmux, SSH, or displayless containers when
that route is enabled. A terminal that does not implement OSC 52 may display the
encoded payload as text. Set `GROK_CLIPBOARD_NO_OSC52=1` before starting ClosedHands to
...
A remote ClosedHands process normally cannot read the local terminal's selection. Use
...
When ClosedHands cannot identify the outer terminal over SSH, it predicts that OSC 52
...
clipboard. Each copy is still saved to a backup file (`~/.closedhands/last-copy.txt` by
...
through `closedhands wrap`, for example `closedhands wrap ssh user@host`. The same command can
wrap container and pod shells. It also restores terminal modes after a dropped
connection.

When an SSH session is not using `closedhands wrap`, ClosedHands shows the one-time tip
...
For repeated SSH use, Doctor offers `closedhands doctor fix ssh-wrap`. It also shows
...
> **Warning**: `closedhands wrap` is experimental and may not work in every setup.
...
Zellij and tmux control mode can limit the alternate screen. ClosedHands normally uses
inline mode in those environments. Run `/doctor` to see the detected condition.
You can configure `[terminal] alt_screen` in `~/.closedhands/pager.toml`, or run
`closedhands --no-alt-screen` to confirm inline mode works.

### Zellij keybindings interfere with ClosedHands

Zellij can intercept Ctrl/Alt keys before they reach ClosedHands. On Zellij 0.41 or
...
mode, if `Ctrl+G` still does not reach ClosedHands, open the command palette and select
...
WezTerm ships with the Kitty keyboard protocol disabled. Run `/doctor` in ClosedHands.
...
keys. ClosedHands therefore does not negotiate the protocol there, and Shift+Enter can
...
If ClosedHands stops receiving mouse input, re-enable mouse reporting in the terminal:
...
After about 10 seconds without a transcript, ClosedHands stops capture and shows
...
to the terminal hosting ClosedHands. Open **System Settings → Privacy & Security →
...
Run `closedhands doctor`, or run `/doctor` while voice mode is on. The **Voice** section
shows the microphone ClosedHands would use. If no input device is available, Doctor

...
ClosedHands supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.closedhands/hooks/`
- **Project hooks** — from `.closedhands/hooks/` in your repo
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.closedhands/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `closedhands plugin marketplace add <source>`)
...
Sources land in `~/.closedhands/config.toml`:
...
Or in `~/.closedhands/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../xai-closedhands-hooks/examples/README.md) — ready-to-use sample hooks

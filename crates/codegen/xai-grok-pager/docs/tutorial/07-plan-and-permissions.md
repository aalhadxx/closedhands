...
ClosedHands asks before doing anything risky — and can plan before it codes.

## Permissions

When ClosedHands wants to run a risky command or edit a file, it pauses and asks:
...
For bigger or more ambiguous tasks, use **plan mode**: ClosedHands explores the
...
`s` requests changes — ClosedHands iterates until you're happy, then implements.
...
background — ClosedHands keeps working and you're notified when it finishes

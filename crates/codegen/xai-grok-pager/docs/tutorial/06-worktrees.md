...
Want ClosedHands working on a feature while you (or another ClosedHands session) work on
...
- **From the shell:**
  ...
  ```bash
  closedhands --worktree=my-feature "refactor the auth module"
  ```
...
- Run two or three ClosedHands sessions on the same repo simultaneously.

//! Headless markdown analysis sharing ClosedHands's exact `pulldown-cmark` config.
//!
...
/// The exact `pulldown-cmark` option set ClosedHands uses to render markdown.
///
...
/// Parse `text` with ClosedHands's options; count elements and flag structural issues.

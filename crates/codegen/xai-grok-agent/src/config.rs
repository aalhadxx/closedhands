...
/// ClosedHands + plan mode toolset WITHOUT subagent tools.
///
...
/// Default ClosedHands toolset + `ask_user_question`.
///
...
impl AgentDefinition {
    ...
    pub fn default_grok_build() -> Self {
        Self::base(
            ...
            "ClosedHands agent for software engineering tasks.",
        )
    }
    /// ClosedHands Concise agent definition — concise output format for SFT/RL.
    pub fn grok_build_concise() -> Self {
        Self {
            ...
            ..Self::base(
                ...
                "ClosedHands agent with concise output format.",
            )
        }
    }
    /// ClosedHands agent with plan mode tools.
    pub fn grok_build_plan() -> Self {
        Self {
            tool_config: grok_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::GrokBuildPlan,
                "ClosedHands agent with plan mode support.",
            )
        }
    }
    /// ClosedHands + plan mode WITHOUT subagent tools.
    pub fn grok_build_plan_no_subagents() -> Self {
        Self {
            ...
            ..Self::base(
                ...
                "ClosedHands agent with plan mode (no subagents).",
            )
        }
    }
    /// Default ClosedHands agent with the `ask_user_question` tool.
    pub fn grok_build_ask_user() -> Self {
        Self {
            tool_config: grok_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::GrokBuildAskUser,
                "ClosedHands agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    /// ClosedHands Orchestrator — GBL model with full GrokBuild tools
